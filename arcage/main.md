...
Rubyでは _Fixnum_ と _Bignum_ の2種類の整数型がありますが，Crystalでは_Fixnum_に相当する固定長整数型だけで _Int8_，_Int16_，_Int32_，_Int64_，_UInt8_，_UInt16_，_UInt32_，_UInt64_ の８種類が存在します。これらのうち型名が _Int_ で始まっているものは符号つき（負の値も扱える）整数型，_UInt_ で始まっているものは符号なし（0以上の値のみを扱う）整数型であることを表し，その後ろの数字で内部的な内部データのサイズ（ビット数）を表しています。さらに，_Bignum_ に相当する多倍長整数型として _BigInt_ が用意されています。
...
そうしてRubyではない「Crystal言語」を意識できれば，その次は「マクロ」や「Cバインディング」といったもう一歩踏み込んだ世界が待っています。Crystalの公式ドキュメント[^crdoc]やAPIリファレンス[^API]は新しい機能について知るのに役立ちますし，仮にそちらで十分な情報が得られなかったとしてもCrystalはそれ自体がCrystal言語で書かれています。公式のGitHubリポジトリのソースツリー[^crsrc]を覗けば，最高のサンプルコードが大量に手に入りますよ。
